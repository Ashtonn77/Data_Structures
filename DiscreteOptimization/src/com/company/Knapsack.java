...
//        boolean temp = (float)o1.value / (float)o1.weight > (float)o2.value / (float)o2.weight; 
       ...
       // return temp ? -1 : 1;
        ...
        return (int)(((float)o1.value / (float)o1.weight) - ((float)o2.value / (float)o2.weight));
...
public class Knapsack {
    ...
    static float getUpperBound(float tWeight, float tValue, int idx, Item[] arr){
        ...
        for (int i = idx; i < arr.length; i++){
            ...
            if(weight + arr[i].weight <= capacity){
                ...
                value -= (float)((capacity - weight) * ((float)arr[i].value / (float)arr[i].weight));
                break;
    ...
    public static void solve(Item[] arr){
        ...
        while(!priorityQueue.isEmpty()){
            ...
            if(current.upperBound > minimumLowerBound 
...
//            minimumLowerBound = Math.min(minimumLowerBound, right.lowerBound);
            minimumLowerBound = Math.min(minimumLowerBound, Math.min(left.lowerBound, right.lowerBound));
